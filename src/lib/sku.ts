  ...
  if (existing) {
    const [lastLog, sold] = await Promise.all([
      ...
      prisma.orderItem.aggregate({
        where: { product: { sku: existing.sku }, order: { status: { in: ["CONFIRMADO", "EN_TRANSITO", "EN_GUATEMALA", "ENVIADO", "ENTREGADO"] } } },
        _sum: { quantity: true },
      }),
    ]);
  ...
  const rows = await Promise.all(
    skus.map(async (sku) => {
      ...
      const sold = await prisma.orderItem.aggregate({
        where: { product: { sku }, order: { status: { in: ["CONFIRMADO", "EN_TRANSITO", "EN_GUATEMALA", "ENVIADO", "ENTREGADO"] } } },
        _sum: { quantity: true },
      });
